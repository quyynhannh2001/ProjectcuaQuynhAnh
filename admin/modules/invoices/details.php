...
<?php 
 ...
 ?>
	...
	<p>Tình trạng: <?php
	 ...
	 <?php  
	 	if ($status == 0) {
	 		echo "<a href = 'index.php?module=invoices&action=accept&id=$id_invoice'>Duyệt đơn hàng</a>";
	 	}
	 	else if($status == 1){
	 		echo "<a href='index.php?module=invoices&action=success&id=$id_invoice'>Giao thành công</a>";
	 	}
	 ?>
			...
			if ($result == false) {
				...
				foreach($result as $row){
					...
					echo "<tr>";
						...
						echo "<td>";
							$id_product = $row['id'];
							echo "<a href='index.php?module=products&action=edit&id=$id_product'>".$row['name']."</a>";
						echo "</td>";
