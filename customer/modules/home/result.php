<?php 
	...
	$sql = "SELECT id, name, url, price FROM products LIMIT $offset,$limit";
 ...
 <?php 
 	if(isset($_GET['key'])){
 		$key=$_GET['key'];
 		$sql = "SELECT id, name, url, price FROM products WHERE name LIKE '%$key%' ";
 		// echo $sql;
 		$result = mysqli_query($conn, $sql);
	if (!$result) {
		$error = "Error: ".mysqli_error($conn);
...
<div id="list_canhan">
	...
	<br>
		<a href="index.php?module=home&action=result&key=<?php echo $key; ?>&page=<?php if($page > 1) {echo ($page-1);} else echo $page; ?>" ><i class="fa fa-arrow-left" style="font-size:24px"></i></a>
	    <b style="font-size: 25px;"><?php echo $page; ?></b>
	    <a href="index.php?module=home&action=result&key=<?php echo $key; ?>&page=<?php if($page < $tong_so_trang) {echo ($page+1);} else echo $page; ?>"><i class="fa fa-arrow-right" style="font-size:24px"></i></a>
</div>
